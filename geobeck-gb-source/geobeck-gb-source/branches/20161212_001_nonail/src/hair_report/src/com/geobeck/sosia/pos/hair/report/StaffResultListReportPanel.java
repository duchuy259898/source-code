					...
					extends FocusTraversalPolicy
	{
        ...
        private void setTotalSalesInfo(JExcelApi jx) {
            ...
            try {
                ...
                sql.append("     ,coalesce(sum(case when product_division in (1,2) then discount_detail_value_in_tax - discount_detail_value_no_tax else 0 end), 0) as Tax");
                sql.append("     ,count( distinct");
                sql.append("                 case");
                sql.append("                     when product_division = 1");
                sql.append("                     then slip_no else null");
                sql.append("                 end");
        ...
        private int setChargeStaffSalesInfo(JExcelApi jx, int row) {
            ...
            if (rdoTaxBlank.isSelected()) {
                // 税抜
                valueString = "discount_detail_value_no_tax";
                claimValueString = "sign(price / (1.0 + get_tax_rate(a.sales_date))) * ceil(abs(price / (1.0 + get_tax_rate(a.sales_date))))";
            }else{
                // 税込
                valueString = "discount_detail_value_in_tax";
