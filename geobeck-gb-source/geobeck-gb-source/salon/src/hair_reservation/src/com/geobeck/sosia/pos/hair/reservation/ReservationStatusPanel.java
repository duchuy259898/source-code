...
//			}
		...
		if(  evt.getClickCount() == 2  )
		{
			...
			if(stay.getSelectedRowCount() > 0 ){
                                  DataReservation dr = rs.getDatas().get(stay.getSelectedRow());
                                  rs.setSlipNo(dr);
                                //IVS_LVTU start add 2017/10/03 #26815 [gb]本部システムでの新規会計はできないようにする
                                if(rs.getDatas().get(stay.getSelectedRow()).getStatus() ==2 && SystemInfo.getCurrentShop().getShopID().equals(0)){
                                      MessageDialog.showMessageDialog(
                                          this,
                                          "本部システムから新規お会計の登録は行えません。",
                                          this.getTitle(), JOptionPane.OK_OPTION);
                                      return;
                                }
                                //IVS_LVTU end add 2017/10/03 #26815 [gb]本部システムでの新規会計はできないようにする
...
//		salesTotal.setValueAt(rs.getTotal().getTotal(), 0, 4);
	...
	private String getReservationToolTip(int row)
	{
                ...
                // IVS start add 20220907 カスタマイズ箇所・２：フロント業務＞＞フロント
                if (SystemInfo.getDatabase().equals("pos_hair_sunaocl")) {
                    TimeSchedule ts = new TimeSchedule();
                    String sql = ts.getNextReservationSql(dr.getShop().getShopID(), cus.getCustomerID());
                    try
                    {
                        ResultSetWrapper rsNextReservation = SystemInfo.getConnection().executeQuery(sql);
                        int dataCount = 0;
                        while (rsNextReservation.next()) {
                            if (dataCount == 0) {
                                sb.append("<tr align=left><td><b>【将来の予約情報】</b></td></tr>");
                            }
                            sb.append("<tr align=left><td><b>[予約日]</b>　" + DateUtil.format(rsNextReservation.getDate("reservation_date"), "M月d日") + "</td></tr>");
                            sb.append("<tr align=left><td><b>[予約時間]</b>　" + rsNextReservation.getString("reservation_time") + "</td></tr>");
                            sb.append("<tr align=left><td><b>[予約メモ]</b></td></tr>");
                            sb.append("<tr align=left><td>" + formatHTMLString(insertNewLineString(rsNextReservation.getString("comment"))) + "</td></tr>");
                            sb.append("<tr><td></td></tr>");

                            dataCount++;
                        }

                        rsNextReservation.close();
                    }
                    catch(SQLException e)
                    {
                        SystemInfo.getLogger().log(Level.SEVERE, e.getLocalizedMessage(), e);
                ...
                // IVS end add 20220907 カスタマイズ箇所・２：フロント業務＞＞フロント
	...
	private void deleteReservation()
	{
		...
		int index = reservation.getSelectedRow();
                ...
                if(index <= rs.getDatas().size()) {
                    ...
                    if (this.getSelectedShop().getUseCancelFlag() != null
                        && this.getSelectedShop().getUseCancelFlag() == 1) {
                    ...
                    if (rs.deleteReservation(reservation.getSelectedRows(), paramConfirm)) {
                            this.showData();
...
*/
	...
	public void setDate(java.util.Date d){
        ...
        private void printMeetingList4PRIMO() {
            JExcelApi jx = new JExcelApi("ミーティング表");
            jx.setTemplateFile("/reports/primo.xls");
			...
			jCal.setTime(date.getDate());
			int month = jCal.get(JapaneseCalendar.MONTH) + 1;
			...
			int allCnt = 0;
            for (int i = 0; i < reservation.getRowCount(); i++) {
				allCnt += reservation.getValueAt(i, 5).toString().split("、").length;
			}
            jx.insertRow(row, allCnt-1);
            // データセット
            for (int i = 0; i < reservation.getRowCount(); i++) {
				Map<String, Object> dataMap = new HashMap<>();
				DataReservation dataReservation = rs.getDatas().get(i);
				int shopID = dataReservation.getShop().getShopID();
				int customerID = dataReservation.getCustomer().getCustomerID();
				int reservationNo = dataReservation.getReservationNo();
				int rc = 0;
				try {
	                StringBuilder sql = new StringBuilder(1000);
					sql.append("select distinct\n" +
						...
						"  , mcus.customer_name2\n" +
						"  , drd.course_flg\n" +
						"  , CASE WHEN drd.course_flg = 2 THEN crs.course_name\n" +
						"         WHEN drd.course_flg = 1 THEN mc.course_name\n" +
						"         ELSE mt.technic_name\n" +
						"    END AS technic_name\n" +
						"  , crs.consumption_num\n" +
						"  , crs.product_num\n" +
						"  , next_reserve\n" +
						...
						"from\n" +
						"  data_reservation dr \n" +
						"  left outer join data_reservation_detail drd \n" +
						"    on dr.shop_id = drd.shop_id \n" +
						"    and dr.reservation_no = drd.reservation_no and drd.delete_date is null \n" +
						"  left outer join mst_technic mt\n" +
						"    on mt.technic_id = drd.technic_id\n" +
						"  left outer join mst_course mc\n" +
						"    on mc.course_id = drd.technic_id\n" +
						"  left outer join mst_customer mcus\n" +
						"    on mcus.customer_id = dr.customer_id\n" +
						//残回数
						"left outer join (\n" +
						"  select\n" +
						"    ds.shop_id as contract_shop_id\n" +
						"  , dc.contract_no\n" +
						"  , dc.contract_detail_no\n" +
						"  , dc.product_id\n" +
						"  , mc.course_name\n" +
						"  , sum(coalesce(dcd.product_num, 0)) as consumption_num\n" +
						"  , dc.product_num\n" +
						"from\n" +
						"  data_sales ds \n" +
						"  inner join data_contract dc \n" +
						"    on ds.shop_id = dc.shop_id \n" +
						"    and ds.slip_no = dc.slip_no \n" +
						"  inner join mst_course mc \n" +
						"    on mc.course_id = dc.product_id \n" +
						"  left join data_contract_digestion dcd \n" +
						"    on dc.shop_id = dcd.contract_shop_id \n" +
						"    and dc.contract_no = dcd.contract_no \n" +
						"    and dc.contract_detail_no = dcd.contract_detail_no \n" +
						"    and dcd.delete_date is null\n" +
						"where\n" +
						...
						"  , dc.product_num\n" +
						"  , mc.course_id\n" +
						"  ) crs on crs.contract_no = drd.contract_no\n" +
						"  and crs.contract_detail_no = drd.contract_detail_no\n" +
						"  and crs.contract_shop_id = drd.contract_shop_id\n" +
						//次回予約
						"left outer join (\n" +
						"	select\n" +
						"	dr.shop_id,\n" +
						"	min(reservation_datetime) as next_reserve\n" +
						"	from\n" +
						"	data_reservation dr\n" +
						"	  left outer join data_reservation_detail drd \n" +
						"	    on dr.shop_id = drd.shop_id \n" +
						...
						"	and to_char(reservation_datetime, 'yyyymmdd') > \n" +
						"	(select to_char( max(reservation_datetime), 'yyyymmdd')\n" +
						"	from data_reservation dr\n" +
						"	  left outer join data_reservation_detail drd \n" +
						"	    on dr.shop_id = drd.shop_id and dr.reservation_no = drd.reservation_no \n" +
						"	where\n" +
						"	  dr.reservation_no = "+reservationNo+"\n" +
						"	)\n" +
						"	group by dr.shop_id\n" +
						") ndr on dr.shop_id = ndr.shop_id\n" +
						//WEBSHOP登録
						"left outer join (\n" +
						" select free_heading_id, customer_id\n" +
						" from mst_customer_free_heading\n" +
						" where customer_id = "+customerID+"\n" +
						" and free_heading_class_id = 5\n" +
						") cfh on cfh.customer_id = dr.customer_id\n" +
						"\n" +
						//フェース登録
						"left outer join (\n" +
						" select free_heading_id as fhid2, customer_id\n" +
						" from mst_customer_free_heading\n" +
						" where customer_id = "+customerID+"\n" +
						" and free_heading_class_id = 6\n" +
						") cfh2 on cfh2.customer_id = dr.customer_id\n" +
						"\n" +
						//化粧品
						"left outer join (\n" +
						" select\n" +
						" ds.shop_id,\n" +
						" ds.customer_id,\n" +
						" max(ds.sales_date) as cosm_date\n" +
						" from\n" +
						" data_sales ds\n" +
						" left outer join data_sales_detail dsd on ds.shop_id = dsd.shop_id and ds.slip_no = dsd.slip_no\n" +
						" left outer join mst_item mi on mi.item_id = dsd.product_id \n" +
						" where\n" +
						"ds.delete_date is null\n" +
						" and dsd.product_division = 2\n" +
						" and mi.item_class_id not in (5,20,14,22,6,19,7,25,24,28,8,17,75,27,77,78,9,71)\n" +
						" and ds.delete_date is null\n" +
						" and ds.customer_id = "+customerID+"\n" +
						" group by \n" +
						" ds.shop_id,\n" +
						" ds.customer_id\n" +
						") cosm on cosm.shop_id = dr.shop_id and cosm.customer_id = dr.customer_id\n" +
						//ファストザイム
						" left outer join (\n" +
						" select\n" +
						" ds.shop_id,\n" +
						" ds.customer_id,\n" +
						" max(ds.sales_date) as fast_date\n" +
						" from\n" +
						" data_sales ds\n" +
						"left outer join data_sales_detail dsd on ds.shop_id = dsd.shop_id and ds.slip_no = dsd.slip_no\n" +
						"left outer join mst_item mi on mi.item_id = dsd.product_id \n" +
						...
						"ds.delete_date is null\n" +
						"and dsd.product_division = 2\n" +
						"and mi.item_class_id = 20\n" +
						"and ds.delete_date is null\n" +
						"and ds.customer_id = "+customerID+"\n" +
						"group by \n" +
						"ds.shop_id,\n" +
						"ds.customer_id\n" +
						") fast on fast.shop_id = dr.shop_id and fast.customer_id = dr.customer_id\n" +
						//サプリ
						"left outer join (\n" +
						"select\n" +
						"ds.shop_id,\n" +
						"ds.customer_id,\n" +
						"max(ds.sales_date) as sup_date-- item_name, dsd.product_division\n" +
						"from\n" +
						"data_sales ds\n" +
						"left outer join data_sales_detail dsd on ds.shop_id = dsd.shop_id and ds.slip_no = dsd.slip_no\n" +
						"left outer join mst_item mi on mi.item_id = dsd.product_id \n" +
						...
						"ds.delete_date is null\n" +
						"and dsd.product_division = 2\n" +
						"and mi.item_class_id = 5\n" +
						"and ds.delete_date is null\n" +
						"and ds.customer_id = "+customerID+"\n" +
						"group by \n" +
						"ds.shop_id,\n" +
						"ds.customer_id --item_name,dsd.product_division\n" +
						") sup on sup.shop_id = dr.shop_id and sup.customer_id = dr.customer_id\n" +
						//売掛金
						"left outer join (\n" +
						"select dpm.customer_id, sum(bill_value) as bill_value\n" +
						"from \n" +
						"data_payment dp\n" +
						"join (\n" +
						"select\n" +
						"  dp.shop_id,dp.slip_no, ds.customer_id, max(dp.payment_no) as payment_no\n" +
						"from\n" +
						"  data_payment dp \n" +
						"  join data_sales ds on \n" +
						"  dp.shop_id = ds.shop_id \n" +
						"  AND dp.slip_no = ds.slip_no \n" +
						"where\n" +
						"      dp.delete_date IS NULL\n" +
						"  AND ds.delete_date IS NULL\n" +
						"  and ds.sales_date > to_date('20161231', 'yyyymmdd') \n" +
						"  AND ds.customer_id = "+customerID+"\n" +
						"  AND ds.shop_id = "+shopID+"\n" +
						"group by dp.shop_id,dp.slip_no,ds.customer_id ) dpm on\n" +
						"dp.shop_id = dpm.shop_id AND dp.slip_no = dpm.slip_no AND dp.payment_no = dpm.payment_no \n" +
						"group by dpm.customer_id \n"+
						") bill on bill.customer_id = dr.customer_id\n" +
						"where\n" +
						"  dr.shop_id = "+shopID+" \n" +
						"  and dr.reservation_no = "+reservationNo+"\n" +
						"  \n" +
						"order by drd.reservation_detail_no");
					SystemInfo.getLogger().log(Level.INFO, sql.toString());

					//コネクションを取得
					ResultSetWrapper rset = SystemInfo.getConnection().executeQuery(sql.toString());
					...
					while (rset.next()) {
						...
						if (rc==0) {
							...
							jx.setValue(1, row, reservation.getValueAt(i, 1).toString());
							//顧客名
							jx.setValue(2, row, rset.getString("customer_name1")+rset.getString("customer_name2"));

							if (rset.getInt("course_flg") == 0 || rset.getInt("course_flg") == 1) {
								//予約メニュー	予約メニュー全て表示。　消化の場合は「消）コース名」とする
								jx.setValue(3, row, rset.getString("technic_name"));
								//残回数	予約メニューに消化がある場合は、そのメニューの消化回数/契約回数を出力する
								jx.setValue(4, row, " ");
							} else {
								//予約メニュー	予約メニュー全て表示。　消化の場合は「消）コース名」とする
								jx.setValue(3, row, "消）" + rset.getString("technic_name"));
								//残回数	予約メニューに消化がある場合は、そのメニューの消化回数/契約回数を出力する
								//jx.setValue(4, row, (rset.getDouble("product_num")-rset.getDouble("consumption_num"))+"/"+ rset.getInt("product_num"));
								jx.setValue(4, row, (rset.getDouble("consumption_num")+1));
							}
							//1/4アフター	手書きで書くので出力不要
							jx.setValue(5, row, " ");
							//担当者	手書きで書くので出力不要
							jx.setValue(6, row, " ");
							//次回予約日	翌日以降の直近の予約日を出力。未来予約がないときは空欄
							jx.setValue(7, row, rset.getDate("next_reserve"));
							//誕生月	誕生月の有無　→　誕生月なら「●」違うなら「空欄」
							if ( dataReservation.getCustomer().getBirthMonth() != null
									&& dataReservation.getCustomer().getBirthMonth()==month) {
								jx.setValue(8, row, "●");
							}
							//WEBSHOP登録	"その顧客のmst_free_heading_class.free_heading_class_id=5の
							//free_heading_id=1の場合「〇」 free_heading_id=2の場合「×」 free_heading_idが未登録の場合「空欄」
							jx.setValue(9, row, (rset.getInt("free_heading_id") == 1)?"○":
												((rset.getInt("free_heading_id") == 2)?"×":""));
							//フェース登録 10
							jx.setValue(10, row, (rset.getInt("fhid2") == 1)?"○":
												((rset.getInt("fhid2") == 2)?"×":""));
							
							//化粧品	"商品分類「ガウン」「EMSパット」「IF」「B100」「FACE CORE」「フォトフラッシュ」「ドクターキャビエット」「テラローラー」「美容器用品」（mst_item_class=8）以外の商品の購入の最終日付
							//→　「YYYY/MM/DD」　未購入の場合「空欄」"
							jx.setValue(11, row, rset.getDate("cosm_date"));
							//ファストザイム	"商品分類「ファストザイム」（mst_item_class=？）の商品の購入が過去にあるか、ないか
							//→　「YYYY/MM/DD」　未購入の場合「空欄」"
							jx.setValue(12, row, rset.getDate("fast_date"));
							//サプリメント	"商品分類「サプリメント」（mst_item_class=？）の商品の購入が過去にあるか、ないか
							//→　「YYYY/MM/DD」　未購入の場合「空欄」"
							jx.setValue(13, row, rset.getDate("sup_date"));
							//その他コース	"予約メニュー以外でコース契約履歴で終了していないコースがあるか、ないか
							//→　あるなら「〇」ないなら「×」"
							// !! 別SQL
							//未収金	出力店舗の売掛金残を出力する　bill_value
							if (rset.getInt("bill_value")>0) {
								jx.setValue(14, row, rset.getInt("bill_value"));
							}
							//備考　comment
							jx.setValue(16, row, rset.getString("comment"));
							
						} else {
							if (rset.getInt("course_flg") == 0 || rset.getInt("course_flg") == 1) {
								//予約メニュー	予約メニュー全て表示。　消化の場合は「消）コース名」とする
								jx.setValue(3, row+rc, rset.getString("technic_name"));
								//残回数	予約メニューに消化がある場合は、そのメニューの消化回数/契約回数を出力する
								jx.setValue(4, row+rc, " ");
							} else {
								//予約メニュー	予約メニュー全て表示。　消化の場合は「消）コース名」とする
								jx.setValue(3, row+rc, "消）" + rset.getString("technic_name"));
								//残回数	予約メニューに消化がある場合は、そのメニューの消化回数/契約回数を出力する
								//jx.setValue(4, row+rc, (rset.getDouble("product_num")-rset.getDouble("consumption_num"))+"/"+ rset.getInt("product_num"));
								jx.setValue(4, row+rc, rset.getDouble("consumption_num")+1);
							}
						}
						} catch(NullPointerException e) {
							SystemInfo.getLogger().log(Level.INFO, row+" - "+rc);
							SystemInfo.getLogger().log(Level.SEVERE, e.getLocalizedMessage(), e);
						}
						rc++;
					}
...
// 20191205 レイアウト変更により削除
//				try {
//
//					String sql = "select \n" +
//								" sum(active_course) as active_course\n" +
//								"from (\n" +
//								"select\n" +
//								"    ds.shop_id as contract_shop_id\n" +
//								"  , dc.contract_no\n" +
...
//								"  , dc.product_id\n" +
//								"  , mc.course_name\n" +
//								"  , dc.product_num - sum(coalesce(dcd.product_num, 0)) as active_course\n" +
//								"from\n" +
//								"  data_sales ds \n" +
//								"  inner join data_contract dc \n" +
//								"    on ds.shop_id = dc.shop_id \n" +
...
//								"and dr.reservation_no = drd.reservation_no\n" +
//								"where\n" +
//								"drd.contract_shop_id is not null\n" +
//								"and drd.contract_no is not null\n" +
//								"and dr.reservation_no = "+reservationNo+") res\n" +
//								"on\n" +
//								"res.contract_shop_id = course.contract_shop_id\n" +
//								"and res.contract_no = course.contract_no\n" +
//								"and res.contract_detail_no = course.contract_detail_no\n" +
//								"where\n" +
//								"res.contract_no is null";
//					//コネクションを取得
//					ResultSetWrapper rset = SystemInfo.getConnection().executeQuery(sql);
//					SystemInfo.getLogger().log(Level.INFO, sql);
//
//					//SystemInfo.getLogger().log(Level.INFO, sql);
//					if (rset.next()) {
//							//その他コース	"予約メニュー以外でコース契約履歴で終了していないコースがあるか、ないか
//							//→　あるなら「〇」ないなら「×」"
//							jx.setValue(13, row, (rset.getDouble("active_course")>0)?"○":"×");
//					} else {
//							jx.setValue(13, row, "×");
//					}
//					rset.close();
