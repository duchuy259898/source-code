...
//	private MstStaffs				staffs			=	new MstStaffs();			/** スタッフリスト */
    ...
    ArrayList<DataReservation> drList = new ArrayList<DataReservation>();

    public ArrayList<DataReservation> getDrList() {
        return drList;
    }

    public void setDrList(ArrayList<DataReservation> drList) {
        this.drList = drList;
    }
...
//        }
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addComponent(prevButton, javax.swing.GroupLayout.PREFERRED_SIZE, 46, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addGap(18, 18, 18)
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            .addGroup(jPanel1Layout.createSequentialGroup()
                                .addComponent(salesInfoPane, javax.swing.GroupLayout.DEFAULT_SIZE, 220, Short.MAX_VALUE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(freeHeadingInfoPane, javax.swing.GroupLayout.DEFAULT_SIZE, 220, Short.MAX_VALUE))
                            .addGroup(jPanel1Layout.createSequentialGroup()
                                .addGap(67, 67, 67)
    ...
    private void dateItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_dateItemStateChanged
        ...
        noMsgFlag = true;
        //20190905 有効期限切れの日付になったらコース削除
		for (int i = 0; i < rr.getReservation().size(); i++) {
            if (rr.getReservation().get(i).getConsumptionCourse() != null) {
				ConsumptionCourse conCourse = rr.getReservation().get(i).getConsumptionCourse();
				if( conCourse.getValidDate() != null && conCourse.getValidDate().compareTo(date.getDate()) < 0) {
					deleteRow(i);
				}
			}
		}
    ...
    private void nextButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_nextButtonActionPerformed
        this.moveReservations(true);
    }//GEN-LAST:event_nextButtonActionPerformed

    private void prevButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_prevButtonActionPerformed
        this.moveReservations(false);
...
//		clearButton.setVisible( false );
        ...
        if (dr.getMobileFlag() == ts.getMobileFlag() || dr.getMobileFlag() == ts.getMobileCancelFlag()) {
            //IVS_LVTu start edit 2017/10/13 #27840 [gb]ログイン店舗以外のWEB予約は開いても既読にしない
            if(dr.getShop() != null && dr.getShop().getShopID().equals(SystemInfo.getCurrentShop().getShopID())){
                try {
                    ConnectionWrapper con = SystemInfo.getConnection();
            ...
            //IVS_LVTu end edit 2017/10/13 #27840 [gb]ログイン店舗以外のWEB予約は開いても既読にしない
...
//		clearButton.setVisible( false );
    ...
    //IVS_LVTu start add 2018/04/06 GB_Studio
    private void moveReservations(boolean isNext) {

        if (0 < reservations.getRowCount()) {
            reservations.removeRowSelectionInterval(0, reservations.getRowCount() - 1);
        }
        this.techIndex = 0;
        if (isNext) {
            currentIndex ++;
        ...
        DataReservation dr = drList.get(currentIndex);

        if (dr != null) {
            if (2 > dr.getStatus()) {
                receiptButton.setIcon(SystemInfo.getImageIcon("/button/reservation/stay_off.jpg"));
                receiptButton.setPressedIcon(SystemInfo.getImageIcon("/button/reservation/stay_on.jpg"));
                receiptButton.setName("");
            }
            this.LoadReservation( dr );
        }
        this.setEnableChangeStudioButton();
    }

    private void setEnabledPrevNextButton() {

        prevButton.setVisible(drList.size() > 1);
        nextButton.setVisible(drList.size() > 1);
        prevButton.setEnabled(currentIndex > 0);
        nextButton.setEnabled(currentIndex < drList.size() - 1);
    }

    private void setEnableChangeStudioButton() {
    ...
    private void showReservations() {
        ...
        //IVS_LVTu start edit 2018/04/06 GB_Studio 予約登録
        if(!nextButton.isVisible()) {
            chargeStaff.setSelectedIndex(0);
            regStaff.setSelectedIndex(0);
        }
        //IVS_LVTu end edit 2018/04/06 GB_Studio 予約登録
...
//			}
        ...
        */
        ...
        Double num = 0.0;
        Double consumptionNum = 0.0;
        Double reservationNum = 0.0;
...
//                }
                ...
                consumptionNum = Math.ceil(consumptionNum); // 残回数切り上げ
...
//        sql.append("     where  slip_no = dcd.slip_no  and shop_id = dc.shop_id ) IS NULL  ");
    ...
    public Double getTotalRestNumOfCourse(int customerId, int courseId, int contractShopID, int contractNo, int contractDetailNo) {
    ...
    public Double getTotalNumReservationOfCourse(int customerId, int courseId, int contractShopID, int contractNo, int contractDetailNo, Integer reservationNo) {
        //Integer result = 0;
        Double result = 0.0;
        try {
            ConnectionWrapper con = SystemInfo.getConnection();
...
//        //IVS NNTUAN END ADD 20131017
        ...
        for(int i = 0;i<reservations.getRowCount();i++) {
            ...
            if((staff.getStaffID() != null && staff.getStaffID() != null)) {
                ...
                if(tempMinute + minuteLen >= 60) {
                    ...
                    int temp = 0;
                    //IVS_LVTU start edit 2017/10/23 #28479 [gb]スタッフシフト時間内にも関わらず、アラート「シフト時間外です」が表示されるときがある
                    temp = (tempMinute + minuteLen)/60;
                    //IVS_LVTU end edit 2017/10/23 #28479 [gb]スタッフシフト時間内にも関わらず、アラート「シフト時間外です」が表示されるときがある
                    tempHour+=temp;
    ...
    private boolean isSameReservation() {
        for (int i = 0; i < reservations.getRowCount(); i++) {
            try {
                DataReservationDetail drd = rr.getReservation().get(i);
                drd.setReservation(rr.getReservation());
                if (drd.getCourseFlg() == null) {
                    drd.getTechnic().setOperationTime((Integer) reservations.getValueAt(i, 2));
                } else if (drd.getCourseFlg() == 1) {
                    drd.getCourse().setOperationTime((Integer) reservations.getValueAt(i, 2));
                ...
                if(!rr.isSameReservation(SystemInfo.getConnection())) {
                    return false;
                }
            } catch (SQLException ex) {
                Logger.getLogger(RegistReservationDialog.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void reserve() {
        ...
        if(this.getSelectedShop().isStudioFlag()
                && drList.size() > 1
                && checkStatus(false)
                && (!this.isExistsConsumption())) {
            result = MessageDialog.showYesNoDialog(
                        this,
                        "同一時間、担当者、メニューの予約について同様に変更しますか？",
                        this.getTitle(),
                        JOptionPane.WARNING_MESSAGE);
        }

        if(result == JOptionPane.YES_OPTION) {
                this.processUpdateRerservationStudio();
        }else {
            this.setData();
    ...
    private void receipt() {
        ...
        if(this.getSelectedShop().isStudioFlag()
            ...
            result = MessageDialog.showYesNoDialog(
                        this,
                        "同一時間、担当者、メニューの予約について全て在店しますか？",
                        this.getTitle(),
                        JOptionPane.WARNING_MESSAGE);
        }

        if(result == JOptionPane.YES_OPTION) {
            this.processUpdateReceiptStudio();
        }else {
            //入力チェック
    ...
    private void delete() {
        ...
        if (rr.getReservation().getShop().getUseCancelFlag() != null
                && rr.getReservation().getShop().getUseCancelFlag() == 1) {
            ConfirmDialog cf = new ConfirmDialog();
            SwingUtil.openAnchorDialog(parentFrame, true, cf, "予約キャンセル処理", SwingUtil.ANCHOR_HCENTER | SwingUtil.ANCHOR_VCENTER);
            ((JDialog) cf.getParent().getParent().getParent().getParent()).dispose();
            if (!cf.paramDeleteFlag) {
                cf = null;
                return;
            } else {
                rr.getReservation().setCancelFlag(cf.paramConfirm);
            }
            cf = null;
        } else {
            //削除確認
        ...
        if(this.getSelectedShop().isStudioFlag()
            ...
            result = MessageDialog.showYesNoDialog(
                    this,
                    "同一時間、担当者、メニューの予約について全て削除しますか？",
                    this.getTitle(),
                    JOptionPane.WARNING_MESSAGE);
        }

        if(result == JOptionPane.YES_OPTION) {
            this.processDeleteStudio();
        }else {
    ...
    //IVS_LVTu start add 2018/04/06 GB_Studio 予約登録
    /**
     * update reservation studio
     */
    private void processUpdateRerservationStudio() {
        ...
        try {
            ...
            try {
                con.begin();
                int index = 0;
                MstCustomer customerIndex = rr.getCustomer();
                MstStaff staffMain = rr.getReservation().getStaff();
                for (DataReservation dr : this.drList) {
                    rr.getReservation().setReservationNo(dr.getReservationNo());
                    setMenuToReservation(rr.getReservation(), dr);
                    rr.getReservation().setStaff(staffMain);
                    if(index == this.currentIndex) {
                        rr.setCustomer(customerIndex);
                    }else {
                        rr.setCustomer(dr.getCustomer());
                    }
                    this.setData();
                    //予約処理
                    if (!rr.registReserveStudio(con)) {
                        isSuccess = false;
                    }
                    index ++;
                }
            ...
            if (isSuccess) {
                ...
                if (SystemInfo.getUserByUserApi() != null && SystemInfo.getUserByUserApi() != 0) {
                    int shopId = rr.reservation.getShop().getShopID();
                    for (DataReservation dr : this.drList) {
                        if (!rr.sendReservationAPI(SystemInfo.getLoginID(), shopId, dr.getReservationNo())) {
                            MessageDialog.showMessageDialog(this,
                                    "媒体との連動ができませんでした。",
                                    this.getTitle(),
                                    JOptionPane.ERROR_MESSAGE);
     ...
     * 受付処理を行う。
     */
    private void processUpdateReceiptStudio() {
        try {
            boolean isSuccess = true;
            ConnectionWrapper con = SystemInfo.getConnection();
            try {
     ...
     * 削除確認
     */
    private void processDeleteStudio() {
     ...
     * @param drTo
     */
    private void setMenuToReservation(DataReservation drFrom, DataReservation drTo) {
        drTo.clear();
        for (DataReservationDetail drd : drFrom) {
            drd.setReservation(drTo);
            drTo.add(drd);
        }
        rr.setReservation(drTo);
    }

    /**
     * check status in studio
     * @return
     */
    private boolean checkStatus(boolean inStore) {
        ...
        for (DataReservation dr : this.drList) {
            ...
            }else if (!this.rr.getReservation().getStatus().equals(dr.getStatus())) {
                return false;
            }
        }
        return true;
    ...
    /**
     * check exists consumption in reservation
     * @return
     */
    private boolean isExistsConsumption() {
        for (DataReservationDetail drd : this.rr.getReservation()) {
            if (drd.getCourseFlg() != null && drd.getCourseFlg().equals(2)) {
                return true;
            }
        }
        return false;
