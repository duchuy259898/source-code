...
public class DataContract extends ArrayList<DataContractDigestion>{
	...
	protected Double remainingNum = null;
       ...
       public void setTaxRate(Double taxRate) {
        ...
        public DataContract(DataSalesDetail dataSalesDetail){
            ...
            this.taxRate = dataSalesDetail.getTaxRate();
      	...
      	public boolean UpdateContractStatus(ConnectionWrapper con) throws SQLException
	{
        ...
        {
            //20170919 GB add start #26335 [GB内対応][gb]タイムゾーンがJST以外だと登録できない
            String strSalesDate = new SimpleDateFormat("yyyy/MM/dd").format(this.getSalesDate());
            //20170919 GB add end #26335
            ...
            if(this.getPraiseTimeLimit().intValue() > 0){               
                //IVS_LVTu start edit 2017/09/08 #25371 [gb]コース契約時の有効期限の値がおかしい
                ...
                //20170919 GB edit start #26335 [GB内対応][gb]タイムゾーンがJST以外だと登録できない
                //sql.append("to_date(to_char(date_trunc('day','").append(this.getSalesDate()).append("'::date) + cast(").append(this.getPraiseTimeLimit().toString()).append(" || ' months' as interval),'YYYY/MM/DD HH24:MI'),'YYYY/MM/DD HH24:MI')-1, \n");
                sql.append("to_date(to_char('").append(strSalesDate).append("'::date + cast(").append(this.getPraiseTimeLimit().toString()).append(" || ' months' as interval),'YYYY/MM/DD HH24:MI'),'YYYY/MM/DD HH24:MI')-1, \n");
                //20170919 GB edit end #26335                
                //IVS_LVTu end edit 2017/09/08 #25371 [gb]コース契約時の有効期限の値がおかしい
            }else{
                sql.append("null, \n");
            }
            ...
            // End add 20131202 lvut
            sql.append(SQLUtil.convertForSQL(this.getTaxRate(), "null")).append(",\n");
	...
	{
                    // 20170828 nami edit start #24273[GB内対応][gb]契約履歴の明細番号取得不備
                    //		return	"select *\n"
                    //				+ "from data_contract\n"
                    //                                + "where shop_id = " + SQLUtil.convertForSQL(shopId) + "\n"
	...
	public String getSelectDataContractSQL()
	{
        ...
        private String getInsertDataContractSQL()
        {
            String sql = "INSERT INTO data_contract\n " 
                        ...
                        + SQLUtil.convertForSQL((this.taxRate != null && this.taxRate != 0) ? this.taxRate : SystemInfo.getTaxRate(validDate)) + ", \n"
                        + "current_timestamp, \n"
                        + "current_timestamp, \n"
                        + "null)";
            return sql;
