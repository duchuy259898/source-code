...
public class MstDataPickupProducts extends ArrayList<MstDataPickupProduct> {
    ...
    public void load(ConnectionWrapper con, boolean existsStaff) throws Exception {
        ResultSetWrapper rs = con.executeQuery(getSelectSQL(existsStaff));
        this.clear();
        while (rs.next()) {
    ...
    public String getSelectSQL(boolean existsStaff) {
        String sql = "";
        sql += "select ds.shop_id , ds.slip_no , dsd.slip_detail_no, ds.customer_id, \n"
               ...
               sql += "             and delete_date is null" + "\n"
				...
				+ ((existsStaff)? " inner ": " left outer")
                + " join mst_staff ms on ms.staff_id = ds.staff_id  " + "\n"
