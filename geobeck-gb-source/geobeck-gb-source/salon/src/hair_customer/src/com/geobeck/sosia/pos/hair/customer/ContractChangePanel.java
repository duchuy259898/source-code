...
public class ContractChangePanel extends AbstractImagePanelEx implements SearchHairProductOpener
{
    ...
    //IVS_LVTu start add 2017/07/11 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
    /**
    * カード
    */
    private final int       PAYMENT_CARD = 2;
    /**
    * 電子マネー
    */
    private final int       PAYMENT_ECARD = 3;
    //IVS_LVTu end add 2017/07/11 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
    ...
    private void registButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_registButtonActionPerformed
        ...
        try {
            ...
            for (int i = 0; i < model.getRowCount(); i++) {
                ...
                //IVS_LVTu start edit 2017/07/13 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
                //dContract.setServiceCharge(Integer.valueOf(serviceChargeText.getText()));
                dContract.setServiceCharge(getServiceCharge());
                //IVS_LVTu end edit 2017/07/13 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
                ...
                dContract.setTaxRate(ia.getTaxRate());
                //IVS_LVTu end add 2015/12/28 Bug #45225
                ...
                dsDetail.setTaxRate(ia.getTaxRate());
            ...
            if (model.getRowCount() > 0) {
                ...
                dsDetail.setProductValue(Long.valueOf(getServiceCharge()));
                //IVS_LVTu end edit 2017/07/13 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
                ...
                //IVS_LVTu start add 2017/07/11 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
                //おつりが発生する場合は、data_payment.change_valueにおつりの額を登録する。
                Long billValue = getBillValue();
                if (billValue > 0 && ia.getPaymentTotal() > billValue) {
                    DataPayment dp = ia.getSales().getPayment(0);
                    dp.setChangeValue(ia.getPaymentTotal() - billValue);
                }
                //IVS_LVTu end add 2017/07/11 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
...
//                }
	...
	public void setDataContract(ResultSetWrapper rs) throws SQLException
	{
		...
		dContract.setStaff( ms ); 
                dContract.setTaxRate( rs.getDouble("tax_rate") ); 
                //IVS_LVTu start delete 2015/06/04 Task #36708
                //arrDS.add(dContract);
                //IVS_LVTu start delete 2015/06/04 Task #36708
	}    
	...
	{               
	    ...
	    sql.append("    left outer join data_contract_digestion dcd");
            //Start edit 2022/03/04 契約変更画面の消化回数が正しくない対応
	    //sql.append(" on dc.shop_id = dcd.shop_id     and dc.contract_no = dcd.contract_no     and dc.contract_detail_no = dcd.contract_detail_no ");
            ...
            //End edit 2022/03/04 契約変更画面の消化回数が正しくない対応
	    sql.append(" WHERE");
     ...
     private void showDataDetail(javax.swing.JTable table,javax.swing.JTable detailTable,int rowIndex)
	{
      ...
      private boolean checkInput() {
        ...
        //請求金額がマイナスの場合
        if (!billValue.equals(ia.getPaymentTotal()) && billValue < 0) {
            MessageDialog.showMessageDialog (
                    this,
                    "請求金額がマイナス（返金）の場合は請求金額とお預り内訳の合計を一致させてください。",
                    this.getTitle(), JOptionPane.ERROR_MESSAGE);
            payments.requestFocusInWindow();
            return false;
        }
        ...
        for (DataPaymentDetail dpd : ia.getSales().getPayment(0)) {
            if (dpd.getPaymentMethod() != null && dpd.getPaymentMethod().getPaymentClass() != null) {
                Integer id = dpd.getPaymentMethod().getPaymentClassID();
                if (id != null && (id.equals(PAYMENT_CARD) || id.equals(PAYMENT_ECARD))) {
                    cardPayment += dpd.getPaymentValue();
                }
            }
        }

        // カードまたは電子マネーの支払額が請求金額より多い場合
        if (billValue > 0 && cardPayment != 0 && billValue < cardPayment) {
            MessageDialog.showMessageDialog (
                    this,
                    "カードまたは電子マネーの支払額が請求金額より多くなっています",
                    this.getTitle(), JOptionPane.ERROR_MESSAGE);
            payments.requestFocusInWindow();
            return false;
        }
        ...
        else if (billValue > 0 && billValue > ia.getPaymentTotal()) {
            if (MessageDialog.showYesNoDialog(this,
    ...
    //IVS_LVTu start add 2017/07/11 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
    /**
     * get bill amount when change course.
     * @return 
     */
    private Long getBillValue() {
        Long value = 0l;
        try {
            value = totalCourseAfter - totalCourseBefore +  serviceCharge;
        } catch (Exception ex) {
            Logger.getLogger(NumberFormatException.class.getName()).log(Level.SEVERE, null, ex);
        }
        return value;
    }
    //IVS_LVTu end add 2017/07/11 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
    //IVS_LVTu start add 2017/07/13 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
    private Integer getServiceCharge(){
        Integer value = 0;
        try {
            if(!"".equals(serviceChargeText.getText().trim())) {
                value = Integer.parseInt(serviceChargeText.getText());
            }
    ...
    //IVS_LVTu end add 2017/07/13 #16151 【契約変更処理画面】売掛になった際に支払額がだぶって登録される
}
