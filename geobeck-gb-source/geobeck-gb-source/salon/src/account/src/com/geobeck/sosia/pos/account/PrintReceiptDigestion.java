...
public class PrintReceiptDigestion extends ArrayList<ReceiptDigestionData> {
    ...
    private java.util.Date salesDate = null; // #43651 201803 GB add
    ...
    // #43651 201803 GB add start 売上日付を表示するようにする
    ...
    // #43651 201803 GB add end 売上日付を表示するようにする
...
//                fullName = this.getCustomer().getFullCustomerName().substring(0, 14);
    ...
    public String getReceiptDigestionSQL(Integer slipNo, Integer shopId ) {
        ...
        sql.append(" select a.*, dcd2.product_sum from ( ");
        sql.append(" select case when ms.display_name is null or ms.display_name = '' ");
        sql.append(" then ms.staff_name1 || ms.staff_name2  else ms.display_name end as staff_name" );
        sql.append(" , dc.product_num as co_product_num, dcd.product_num as cd_product_num ");
        sql.append(" , dc.valid_date,  dcd.contract_no, dcd.contract_shop_id, dcd.contract_detail_no ");
        sql.append(" , mc.course_name as product_name");
        sql.append(" from data_contract_digestion dcd" );
        sql.append(" inner join data_contract dc on dcd.contract_shop_id = dc.shop_id ");
        sql.append(" and dcd.contract_no = dc.contract_no and dcd.contract_detail_no = dc.contract_detail_no ");
        sql.append(" inner join mst_course mc on mc.course_id = dc.product_id ");
	sql.append(" inner join mst_staff ms on ms.staff_id = dcd.staff_id ");
        sql.append(" where dcd.slip_no = ").append(SQLUtil.convertForSQL(slipNo));
        sql.append(" and dcd.shop_id = ").append(SQLUtil.convertForSQL(shopId)).append(" ) a ");
        sql.append(" left join ( SELECT SUM(cd1.product_num) AS product_sum, cd1.contract_no ");
        sql.append(" , cd1.contract_shop_id, cd1.contract_detail_no ");
        sql.append(" FROM data_contract_digestion cd1 WHERE cd1.delete_date IS NULL ");
        sql.append(" GROUP BY cd1.contract_no, cd1.contract_shop_id, cd1.contract_detail_no ) dcd2 ");
        sql.append(" on a.contract_no = dcd2.contract_no and a.contract_shop_id = dcd2.contract_shop_id and a.contract_detail_no = dcd2.contract_detail_no ");
