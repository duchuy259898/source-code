...
public class RegisterCashIOReportPanel extends com.geobeck.sosia.pos.swing.AbstractImagePanelEx {
	...
	private void init()
	{
            ...
            // IVS start edit 20220928 レジ入金表＞西暦プルダウンを売上入力がある西暦まで表示する対応
            //initYearCombo(cmbDayWayYear, cdr.get(Calendar.YEAR));
            List<Integer> yearList = getSalesYear();
            initYearCombo(cmbDayWayYear, yearList);
            // IVS end edit 20220928 レジ入金表＞西暦プルダウンを売上入力がある西暦まで表示する対応
	...
	private void printPaymentAccording()
	{
		...
		MstShop ms = (MstShop)target.getSelectedItem();
            ...
            //メニュー表示設定
            paramBean.setDisplayPriceType(SystemInfo.getAccountSetting().getDisplayPriceType());
            ...
            try {
                ...
                //IVS_LVTU start edit 2017/09/27 #26530 [gb]レジ入金表：レジ入出金のみ場合、レジ入出金の金額が表示されない
                ...
                if (this.rdoTaxBlank.isSelected()) {
                    // 税抜き
                    allPriceSql += "     ,sum(CASE";
                    allPriceSql += "       WHEN change_course_flag > 0 THEN";
                    allPriceSql += "           (SELECT ceil(sum(payment_value/(1.0 + get_tax_rate(a.sales_date))))";
                    allPriceSql += "            FROM data_payment_detail dpd";
                    allPriceSql += "            WHERE dpd.shop_id = b.shop_id";
                    allPriceSql += "              AND dpd.slip_no = b.slip_no)";
                    allPriceSql += "       ELSE 0";
                    allPriceSql += "     END) AS change_course_value";
                    allPriceSql += " ,sum(a.discount_value_no_tax) as all_discount";
                } else if(this.rdoTaxUnit.isSelected()) {
                    // 税込み
                    allPriceSql += "     ,sum(CASE";
                    allPriceSql += "       WHEN change_course_flag > 0 THEN";
                    allPriceSql += "           (SELECT sum(payment_value)";
                    allPriceSql += "            FROM data_payment_detail dpd";
                    allPriceSql += "            WHERE dpd.shop_id = b.shop_id";
                    allPriceSql += "              AND dpd.slip_no = b.slip_no)";
                    allPriceSql += "       ELSE 0";
                ...
                //IVS_LVTU end edit 2017/09/27 #26530 [gb]レジ入金表：レジ入出金のみ場合、レジ入出金の金額が表示されない
                ...
                if (this.rdoTaxBlank.isSelected()) {
                    ...
                    allPriceSql += "             ,sum(case when product_division in (1, 3) then ";
                    allPriceSql += getTechItemCourseValueQuery(paramBean);
                    allPriceSql += " else 0 end) as tech_value";
                    
                    //allPriceSql += "             ,sum(case when product_division in (2, 4) then discount_detail_value_in_tax else 0 end) as item_value";
                    allPriceSql += "             ,sum(case when product_division in (2, 4) then ";
                    allPriceSql += getTechItemCourseValueQuery(paramBean);
                    allPriceSql += " else 0 end) as item_value ";
                    ...
                    allPriceSql += "             ,sum(case when product_division in (5) then ";
                    allPriceSql += getTechItemCourseValueQuery(paramBean);
                    allPriceSql += " else 0 end) as course_value ";
                ...
                mainQuery += " select";
                //IVS_LVTU start edit 2017/09/27 #26530 [gb]レジ入金表：レジ入出金のみ場合、レジ入出金の金額が表示されない
                mainQuery += "     coalesce(all_data.date, register.date, physics.date) as date";
                //IVS_LVTU end edit 2017/09/27 #26530 [gb]レジ入金表：レジ入出金のみ場合、レジ入出金の金額が表示されない
                ...
                //IVS_LVTU edit 2019/09/10 [gb]SPOS増税対応
                ...
                mainQuery +="      WHEN a.overall_discount > 0 THEN discount_detail_value_in_tax - discount_detail_value_no_tax  - (  a.overall_discount -  trunc( ceil((a.overall_discount) / (1.0 + get_tax_rate(a.sales_date))))  )  " + "\n";
                ...
                mainQuery +=" 		and product_division not in(6, 7, 8, 9)  " + "\n";
                //IVS_LVTU end edit 2017/09/27 #26530 [gb]レジ入金表：レジ入出金のみ場合、レジ入出金の金額が表示されない
                ...
                //IVS_LVTU start edit 2017/09/27 #26530 [gb]レジ入金表：レジ入出金のみ場合、レジ入出金の金額が表示されない
                mainQuery += "         full outer join (" + registerInOutSql + ") register";
                mainQuery += "                on all_data.date = register.date";
                mainQuery += "         full outer join (" + physicsCashSql + ") physics";
                mainQuery += "                on coalesce(all_data.date, register.date) = physics.date";
                //IVS_LVTU end edit 2017/09/27 #26530 [gb]レジ入金表：レジ入出金のみ場合、レジ入出金の金額が表示されない
                mainQuery += " order by";
                mainQuery += "     date";
         ...
         * @return 
         */
        private String getTechItemCourseValueQuery(ReportParameterBean paramBean) {
            String sql = "";
            
            if(paramBean.getDisplayPriceType()==1 && paramBean.getDiscountType()==1) {
                // 外税＆税抜き額から割引
                    ...
                    for (int i = 0; i < days; i++) {
                        ...
                        while (rs.next()) {
                            ...
                            if (paBean.getDate().equals(rs.getString("date"))) {
                                ...
                                // IVS start add 20220928 レジ入金表の営業日数のカウントが正しくない対応
                                if (rs.getObject("tech_value") != null
                                    || rs.getObject("item_value") != null
                                    || rs.getObject("course_value") != null) {
                                // IVS end add 20220928 レジ入金表の営業日数のカウントが正しくない対応
                                dateCount++;
                                // IVS start add 20220928 レジ入金表の営業日数のカウントが正しくない対応
                                }
                                // IVS end add 20220928 レジ入金表の営業日数のカウントが正しくない対応
                        ...
                        while (rsPayment.next()) {
                            ...
                            if (paBean.getDate().equals(rsPayment.getString("date"))) {
                                ...
                                //IVS_LVTU start edit 2017/09/27 #26530 [gb]レジ入金表：レジ入出金のみ場合、レジ入出金の金額が表示されない
                                jx.setValue(11, row, rsPayment.getLong("cash_sales_in_tax"));
...
//                                jx.setValue(11, row, cashValue);
	...
	private void printCardSalesDetails()
	{
        ...
        private String getCardSalesHeaderQuery(ReportParameterBean paramBean) {
            ...
            //IVS_LVTu start delete 2017/08/02 #20901 [gb] 支払明細表 削除された支払方法が集計されない
            //sql.append("                and c.delete_date is null");
            //IVS_LVTu end delete 2017/08/02 #20901 [gb] 支払明細表 削除された支払方法が集計されない
    ...
    //LVTu end add 2016/03/18 Bug #49140
        // IVS start add 20220928 レジ入金表＞西暦プルダウンを売上入力がある西暦まで表示する対応
        private List getSalesYear() {
            ...
            sql.append("     and shop_id in (").append(shopIdList).append(")");
            sql.append(" group by");
        ...
        // IVS end add 20220928 レジ入金表＞西暦プルダウンを売上入力がある西暦まで表示する対応
