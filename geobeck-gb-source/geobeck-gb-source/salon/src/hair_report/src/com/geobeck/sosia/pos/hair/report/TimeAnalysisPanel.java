 ...
 * @author geobeck
 */
public class TimeAnalysisPanel extends com.geobeck.sosia.pos.swing.AbstractImagePanelEx {
    ...
    //IVS_LVTu start edit 2016/03/01 New request #48790
    /**
     * 初期化処理を行う。
     */
    public void init() {
        ...
        switch (mss.getTimeAnalysisCondition1()) {
            ...
            case 2:
                ...
                break;
            case 3:
                rdoReservationTime.setSelected(true);
                break;
        }
    ...
    private void showDaily() {
	    ...
	    int dayOfMonth = calTemp.get(Calendar.DAY_OF_MONTH);
            for(int i = 0;i < dayOfMonth-1;i ++) {
                for ( int j = 0;j < daily.getColumnCount();j ++) {
                    addValue(daily, i, j, 0);
                }
            }
        ...
        for (int row = 0; row < daily.getRowCount(); row++) {
            ...
            //dayTotalSales = value;
            dayTotalSales = (value==null?0:value);
            try {
            ...
            //dayTotalCount = value;
            dayTotalCount  = (value==null?0:value);
            try {
            ...
            if (rdoOutValue.isSelected()) {
                ...
                col = MAC_COL - 5;
                try {
                    price = totalSales.doubleValue() / totalCount.doubleValue();
                } catch (Exception e) {
                }
                daily.setValueAt(Math.round(price), row, col);
            }
            ...
            //dayTotalPrev = value;
            dayTotalPrev = (value==null?0:value);
            try {
...
//            }
    ...
    private ResultSetWrapper getDailyData() {
        ...
        } else if (rdoLeaveTime.isSelected()) {
            sql.append("         ," + getTargetTimeColumn("dr.leave_time"));
        } else if (rdoReservationTime.isSelected()) {
            sql.append("         ," + getTargetTimeColumn("drd.reservation_datetime"));
        }

        sql.append("    ,(select coalesce(sum(");
        if (rdoTaxUnit.isSelected()) {
            //sql.append("         ,ds.discount_sales_value_in_tax as sales_value");
            //IVS_Ptthu start edit 20160621 Bug #51954
            sql.append("    a.detail_value_in_tax - a.discount_value");
        } else {
            //sql.append("         ,ds.discount_sales_value_no_tax as sales_value");
            sql.append("    CASE");
            sql.append("    WHEN (a.detail_value_in_tax - a.discount_value - (a.detail_value_no_tax - sign(a.discount_value / (1.0 + get_tax_rate(a.tax_rate, a.sales_date))) * ceil(abs(a.discount_value / (1.0 + get_tax_rate(a.tax_rate, a.sales_date)))))) < 0::numeric THEN a.detail_value_in_tax - a.discount_value - (a.detail_value_in_tax - a.discount_value - (a.detail_value_no_tax - sign(a.discount_value / (1.0 + get_tax_rate(a.tax_rate, a.sales_date))) * ceil(abs(a.discount_value / (1.0 + get_tax_rate(a.tax_rate, a.sales_date))))))");
            sql.append("    ELSE a.detail_value_no_tax - sign(a.discount_value / (1.0 + get_tax_rate(a.tax_rate, a.sales_date))) * ceil(abs(a.discount_value / (1.0 + get_tax_rate(a.tax_rate, a.sales_date))))");
            sql.append("    END ");
        }
	...
	sql.append("    )  AS sales_value");
        ...
        if (rdoReservationTime.isSelected()) {
            sql.append("   JOIN");
            sql.append("   (SELECT min(drd.reservation_datetime) AS reservation_datetime, drd.shop_id, drd.reservation_no");
            sql.append("   FROM data_reservation_detail drd");
            sql.append("   WHERE drd.delete_date IS NULL");
            sql.append("   GROUP BY drd.shop_id,");
            sql.append("           drd.reservation_no) drd on drd.shop_id = dr.shop_id and drd.reservation_no = dr.reservation_no");
        }
        sql.append("         where");
	...
	sql.append("    ))  AS sales_value");
        ...
        sql.append(" from");
        sql.append("     view_data_sales_valid ds");
        //Luc start add 20140715 Request #26709 [gb]PG8.4での動作不備調整
        sql.append("     JOIN data_reservation dr using(shop_id, slip_no) ");
        //Luc end add 20140715 Request #26709 [gb]PG8.4での動作不備調整
        sql.append(" where");
...
//        }
        ...
        for (int col = 0; col < (categoryTotal.getColumnCount() - 2); col++) {
            ...
            for (int row = 0; row < category.getRowCount(); row++) {
                total += Long.valueOf(category.getValueAt(row, col).toString());

            }
            //金額row
            if (col % 2 == 0) {
                try {
                    total -= Long.valueOf(categoryTotal.getValueAt(0, col).toString());
                } catch (Exception e) {
                }
            }
            categoryTotal.setValueAt(total, 1, col);

            //客単価
            if (col % 2 == 0) {
                //金額
                Double d1 = Double.valueOf(categoryTotal.getValueAt(1, col).toString());
                //客数
                Double d2 = Double.valueOf(categoryTotal.getValueAt(2, col) != null ? categoryTotal.getValueAt(2, col).toString() : "0.0");
                //客単価 : 金額/客数
                if(d2 != 0) {
                    categoryTotal.setValueAt(Math.round(d1 / d2), 3, col);
                }else {
                    categoryTotal.setValueAt(0, 3, col);
                }
            }
            ...
            if (col % 2 == 0) {
                try {
                    totalDiscount += Long.valueOf(categoryTotal.getValueAt(0, col).toString());
                } catch (Exception e) {
                }
                try {
                    totalAmount += Long.valueOf(categoryTotal.getValueAt(1, col).toString());
                } catch (Exception e) {
                }

                try {
                    totalCustomer += Long.valueOf(categoryTotal.getValueAt(2, col).toString());
                } catch (Exception e) {
                }
            }
            //数量
            if (col % 2 != 0) {
                try {
                    totalQuantity += Long.valueOf(categoryTotal.getValueAt(1, col).toString());
                } catch (Exception e) {
                }
            }

        }
        categoryTotal.setValueAt(totalDiscount, 0, (categoryTotal.getColumnCount() - 2));
        categoryTotal.setValueAt(totalAmount, 1, (categoryTotal.getColumnCount() - 2));
        categoryTotal.setValueAt(totalQuantity, 1, (categoryTotal.getColumnCount() - 1));

        categoryTotal.setValueAt(totalCustomer, 2, (categoryTotal.getColumnCount() - 2));
        Double d1 = Double.valueOf(totalAmount);
        Double d2 = Double.valueOf(totalCustomer);

        categoryTotal.setValueAt(Math.round(d1 / d2), 3, (categoryTotal.getColumnCount() - 2));
    ...
    private ResultSetWrapper getCategoryHeader() {
        ...
        sql.append("     ,coalesce(coalesce(mtc.technic_class_id, mic.item_class_id), mcc.course_class_id) as class_id");
        sql.append("     ,coalesce(coalesce(mtc.technic_class_name, mic.item_class_name), mcc.course_class_name) as class_name");
        sql.append("     ,coalesce(coalesce(mtc.display_seq, mic.display_seq),mcc.display_seq) as display_seq");
        sql.append(" from");
        ...
        sql.append("         LEFT JOIN mst_course mc ON ds.product_division IN (5,7,8)");
        sql.append("         AND ds.product_id = mc.course_id");
        ...
        if(rdoClassAll.isSelected()) {
            sql.append(" AND ds.product_division <> 6");
        }
    ...
    private String getCategoryBaseSQL() {
        ...
        sql.append("     ,ds.product_division");
        sql.append("     ,coalesce(coalesce(mt.technic_class_id, mi.item_class_id), mc.course_class_id) as class_id");
        ...
        if (rdoTaxUnit.isSelected()) {
            ...
            if(SystemInfo.getAccountSetting().getDisplayPriceType()==1 && SystemInfo.getAccountSetting().getDiscountType()==1){
                    // 税込出力＆外税表示＆税抜き額から割引
                    sql.append(" ,trunc(discount_value * (1 + get_tax_rate(tax_rate, sales_date))) as discount_value");
                } else {
                    sql.append(" ,detail_value_in_tax - discount_detail_value_in_tax as discount_value");
                }
        } else {
        ...
        sql.append("                on ds.product_division in (5, 7, 8)");
        sql.append("               and ds.product_id = mc.course_id");
        if (rdoReservationTime.isSelected()) {
            sql.append("   JOIN");
            sql.append("   (SELECT min(drd.reservation_datetime) AS reservation_datetime, drd.shop_id, drd.reservation_no");
    ...
    private void showWeekly() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                //---- 2016/08/31 GB MOD Start #53332
//                // 客数が存在する場合は営業日のためNULL列をゼロ埋めする
//                for (col = 0; col < MAC_COL; col++) {
                // 客数が存在する場合は営業日のため前年実績、前年度比以外のNULL列をゼロ埋めする
                for (col = 0; col < MAC_COL - 4; col++) {
                //---- 2016/08/26 GB MOD End #53332
...
//        sql.append("      translate(date_part('dow', sales_date), 0, 7) as weekly_id");
        sql.append("      translate(to_char(date_part('dow', sales_date),'9'), '0', '7') as weekly_id");
...
//---- 2013/04/24 GB MOD END
	...
	sql.append("    ))  AS sales_value");
        ...
        sql.append(" from");
        sql.append("     view_data_sales_valid ds");
        //Luc start add 20140715 #26709 [gb]PG8.4での動作不備調整
        //2016/08/26 GB MOD Start #53332
        //sql.append("     JOIN data_reservation dr using(shop_id, slip_no)");
        //2016/08/26 GB MOD End #53332
        //Luc end add 20140715 Request #26709 [gb]PG8.4での動作不備調整
        sql.append(" where");
...
//        sql.append("      translate(date_part('dow', manage_date), 0, 7) as weekly_id");
        sql.append("      translate(to_char(date_part('dow', manage_date),'9'), '0', '7') as weekly_id");
//---- 2013/04/24 GB MOD END
        sql.append("     ,0");
        sql.append("     ,0");
        sql.append(" from");
    ...
    private ResultSetWrapper getWeeklyDataPrev() {
        ...
        calStart.setTime(startDate.getDate());
        calStart.add(Calendar.YEAR, -1);
//---- 2016/08/05 GB MOD Start #53332
        //Calendar calEnd = (Calendar) calStart.clone();
        //calEnd.set(Calendar.DAY_OF_MONTH, calEnd.getActualMaximum(Calendar.DAY_OF_MONTH));
        Calendar calEnd = Calendar.getInstance();
        calEnd.setTime(endDate.getDate());
        calEnd.add(Calendar.YEAR, -1);
//---- 2016/08/05 GB MOD End #53332
...
//---- 2013/04/24 GB MOD END
        ...
        if (rdoTaxUnit.isSelected()) {
            ...
            //IVS_Ptthu end edit 20160621 Bug #51954
            sql.append("    END ");
        }
        sql.append("    ),0)AS sales_value");
	sql.append("    from view_data_sales_detail_valid a");
    ...
    //IVS_LVTu end edit 2016/03/01 New request #48790
    /**
    ...
    private void initComponents() {
        ...
        rdoReservationTime = new javax.swing.JRadioButton();
        ...
        rdoVisitTime.setBounds(440, 10, 80, 20);
        ...
        rdoStartTime.setBounds(520, 10, 90, 20);
        ...
        rdoLeaveTime.setBounds(620, 10, 80, 20);
        ...
        rdoTaxUnit.setBounds(360, 80, 50, 20);
        ...
        rdoTaxBlank.setBounds(440, 80, 50, 20);
        ...
        rdoTimeFixed.setBounds(360, 33, 60, 20);
        ...
        btnTimeSetting.setBounds(700, 20, 92, 25);
        ...
        showButton.setBounds(590, 60, 92, 25);
        ...
        btnOutput.setBounds(700, 60, 92, 25);
        ...
        rdoClassAll.setBounds(70, 65, 60, 20);
        ...
        rdoClassTech.setBounds(130, 65, 80, 20);
        ...
        rdoClassItem.setBounds(210, 65, 80, 20);
        ...
        rdoOutValue.setBounds(360, 56, 50, 20);
        ...
        rdoOutCount.setBounds(440, 56, 50, 20);

        timeGroup.add(rdoReservationTime);
        rdoReservationTime.setSelected(true);
        rdoReservationTime.setText("予約時間");
        rdoReservationTime.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        rdoReservationTime.setFocusCycleRoot(true);
        rdoReservationTime.setMargin(new java.awt.Insets(0, 0, 0, 0));
        rdoReservationTime.setOpaque(false);
        pnlMain.add(rdoReservationTime);
        rdoReservationTime.setBounds(360, 10, 80, 20);
        ...
        daily.getColumnModel().getSelectionModel().setSelectionMode(javax.swing.ListSelectionModel.SINGLE_INTERVAL_SELECTION);
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .add(org.jdesktop.layout.GroupLayout.TRAILING, jPanel1Layout.createSequentialGroup()
                ...
                .add(17, 17, 17))
            .add(dailyScroll, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
        );
        ...
        jPanel2Layout.setHorizontalGroup(
            ...
            .add(org.jdesktop.layout.GroupLayout.TRAILING, jPanel2Layout.createSequentialGroup()
                ...
                .add(17, 17, 17))
            .add(categoryScroll, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
        );
        ...
        jPanel3Layout.setHorizontalGroup(
            ...
            .add(org.jdesktop.layout.GroupLayout.TRAILING, jPanel3Layout.createSequentialGroup()
                ...
                .add(17, 17, 17))
            .add(weeklyScroll, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
        );
    ...
    private void timeTabStateChanged(javax.swing.event.ChangeEvent evt) {//GEN-FIRST:event_timeTabStateChanged
        ...
        //日別集計で対象期間From~ToのToの日付が入力不可なのにキーボード「del」で消せる。
        if( timeTab.getSelectedIndex() == 0) {
    ...
    private void showButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_showButtonActionPerformed
        ...
        try {
            ...
            if (rdoReservationTime.isSelected()) {
                mss.setTimeAnalysisCondition1(3);
            }
    ...
    private javax.swing.JRadioButton rdoReservationTime;
    ...
    private TimeAnalysisFocusTraversalPolicy ftp =
            new TimeAnalysisFocusTraversalPolicy();
                ...
                for (MstTimeSetting eachshift : shiftsAllInShop) {
                    ...
                    if (strShiftName != null && strShiftName.length() > 0) {
                        ...
                        if (iShift >= 0) {
                            ...
                            //IVS_LVTu start add 2016/03/14 Bug #49036
                            //「時間設定」画面、時間設定のFromが「00」時の場合、設定されていないことになる。
    ...
    private void setDailyTableColumnWidth(JTableEx table, int startIndex) {
        ...
        table.getColumnModel().getColumn(table.getColumnCount() - 2).setPreferredWidth(60);
    ...
    private String getTargetTimeName() {
        ...
        if (rdoReservationTime.isSelected()) {
            return rdoReservationTime.getText();
        } else if (rdoVisitTime.isSelected()) {
    ...
    private String getTargetTimeColumn(String col) {
        ...
        if (rdoReservationTime.isSelected()) {
            sql.append("  and drd.reservation_datetime::date > ds.sales_date::date ");
        }
        if (rdoVisitTime.isSelected()) {
    ...
    public TimeAnalysisFocusTraversalPolicy getFocusTraversalPolicy() {
        return ftp;
    }
     ...
     */
    private class TimeAnalysisFocusTraversalPolicy
        ...
        public Component getComponentAfter(Container aContainer,
            ...
            if (aComponent.equals(shop)) {
                return startDate;
            } else if (aComponent.equals(startDate)) {
                if (endDate.isEditable()) {
                    return endDate;
                }
                return this.getSelectedClassType();
            } else if (aComponent.equals(endDate)) {
                return this.getSelectedClassType();
            } else if (aComponent.equals(rdoClassAll)
                    || aComponent.equals(rdoClassTech)
                    || aComponent.equals(rdoClassItem)) {
                return this.getSelectedTargetType();
            } else if (aComponent.equals(rdoVisitTime)
                    || aComponent.equals(rdoStartTime)
                    || aComponent.equals(rdoLeaveTime)) {
                return this.getSelectedTimeType();
            } else if (aComponent.equals(rdoTimeFixed)
                    || aComponent.equals(rdoTimeCustom)) {
                return this.getSelectedOutputType();
            } else if (aComponent.equals(rdoOutValue)
                    || aComponent.equals(rdoOutCount)) {
                return this.getSelectedTaxType();
            }
            return this.getSelectedFirst();
        }
        ...
        private Component getSelectedClassType() {
            if (rdoClassAll.isEnabled() && rdoClassAll.isSelected()) {
                return rdoClassAll;
        ...
        private Component getSelectedTargetType() {
            if (rdoVisitTime.isSelected()) {
                return rdoVisitTime;
            } else if (rdoStartTime.isSelected()) {
                return rdoStartTime;
        ...
        private Component getSelectedTimeType() {
            if (rdoTimeFixed.isSelected()) {
                return rdoTimeFixed;
            }
        ...
        private Component getSelectedOutputType() {
            if (rdoOutValue.isEnabled() && rdoOutValue.isSelected()) {
                return rdoOutValue;
            } else if (rdoOutCount.isEnabled() && rdoOutCount.isSelected()) {
                return rdoOutCount;
            }
            return this.getSelectedTaxType();
        }

        private Component getSelectedFirst() {
            if (shop.getItemCount() == 1) {
                return startDate;
            } else {
                return shop;
