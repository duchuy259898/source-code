...
public class OriginalReport01Logic extends OriginalReportCommonLogic {
    ...
    static final Map<String, String> COLUMN_OUTPUT_MAP = new HashMap<String, String>() {
        {
            ...
            this.put(OUTPUT_PERIOD_COLUMNS[1], "R3"); // 201710 GB #27108 R3追加
            this.put(OUTPUT_PERIOD_COLUMNS[2], "R（既存再来）");
        }
    };
    
    static final String[] OUTPUT_UNDERFIVE_COLUMNS = {
         "underfive 数量"
        , "underfive【プラチナ割引】数量"
    };
    ...
    class OriginalReport01Bean implements Serializable {
        ...
        public OriginalReport01DetailBean getSummaryDetail() {
            ...
            if (this.getDetail().isEmpty()) {
                ...
                empty.setR3(new OriginalListBean(null, 0L)); // 201710 GB add #27108 R3追加
                ...
                empty.setTwiceSalesNum(new OriginalListBean(null, 0L)); // 201710 GB add #27108 R3追加
                empty.setFixedSalesNum(new OriginalListBean(null, 0L));
                // IVS_LVTu start edit 20210129 #18378 underfive 数量追加
                empty.setUnderfiveQuantity(new OriginalListBean(null, 0L));
                empty.setUnderfiveDiscountQuantity(new OriginalListBean(null, 0L));
                // IVS_LVTu end edit 20210129 #18378 underfive 数量追加
                tmp.addBean(empty);
            } else {
    ...
    class OriginalReport01DetailBean implements Serializable {
        ...
        // 201710 GB add #27108 R3追加
        /** R3 */
        private OriginalListBean r3 = null;

        /** 新規顧客数 */
        private OriginalListBean newSalesNum = null;

        // 201710 GB add #27108 R3追加
        private OriginalListBean newTwiceNum = null;
        ...
        /** underfive 数量 */
        private OriginalListBean underfiveQuantity = null;
        
         /** underfive【プラチナ割引】数量 */
        private OriginalListBean underfiveDiscountQuantity = null;
        ...
        // 201710 GB add start #27108 R3追加
        public OriginalListBean getR3() {
            return r3;
        }
        public void setR3(OriginalListBean r3) {
            this.r3 = r3;
        }
        // 201710 GB add end #27108 R3追加
        ...
        // 201710 GB add start #27108 R3追加
        public OriginalListBean getTwiceSalesNum() {
            return newTwiceNum;
        }
        public void setTwiceSalesNum(OriginalListBean newTwiceNum) {
            this.newTwiceNum = newTwiceNum;
        }
        // 201710 GB add end #27108 R3追加
        ...
        /**
         * @return the underfiveQuantity
         */
        public OriginalListBean getUnderfiveQuantity() {
            return underfiveQuantity;
        }

        /**
         * @param underfiveQuantity the underfiveQuantity to set
         */
        public void setUnderfiveQuantity(OriginalListBean underfiveQuantity) {
            this.underfiveQuantity = underfiveQuantity;
        }

        /**
         * @return the underfiveDiscountQuantity
         */
        public OriginalListBean getUnderfiveDiscountQuantity() {
            return underfiveDiscountQuantity;
        }

        /**
         * @param underfiveDiscountQuantity the underfiveDiscountQuantity to set
         */
        public void setUnderfiveDiscountQuantity(OriginalListBean underfiveDiscountQuantity) {
            this.underfiveDiscountQuantity = underfiveDiscountQuantity;
        }
        ...
        public void addBean(OriginalReport01DetailBean bean) {
            ...
            // 201710 GB add start #27108 R3追加
            if (this.r3 == null) {
                this.r3 = new OriginalListBean();
            }
            this.r3.addBean(bean.getR3());

            if (this.newTwiceNum == null) {
                this.newTwiceNum = new OriginalListBean();
            }
            this.newTwiceNum.addBean(bean.getTwiceSalesNum());
            // 201710 GB add end #27108 R3追加
            ...
            // IVS_LVTu start edit 20210129 #18378 underfive 数量追加
            if (this.underfiveQuantity == null) {
                this.underfiveQuantity = new OriginalListBean();
            }
            this.underfiveQuantity.addBean(bean.getUnderfiveQuantity());
            
            if (this.underfiveDiscountQuantity == null) {
                this.underfiveDiscountQuantity = new OriginalListBean();
            }
            this.underfiveDiscountQuantity.addBean(bean.getUnderfiveDiscountQuantity());
            // IVS_LVTu end edit 20210129 #18378 underfive 数量追加
        }
    ...
    protected Result process(ConnectionWrapper connection) throws Exception {
        ...
        // IVS_LVTu start edit 20210129 #18378 underfive 数量追加
        Map<Integer, Map<String, DataUnderfive>> underfiveNumMap = getUnderfiveNum(connection, shopIdList, staffIdList);
        // IVS_LVTu end edit 20210129 #18378 underfive 数量追加
...
//                System.out.println(System.currentTimeMillis() - start);
                ...
                // IVS_LVTu start edit 20210129 #18378 underfive 数量追加
                // underfive数量
                this.setUnderfiveNum(underfiveNumMap, row);
                // IVS_LVTu end edit 20210129 #18378 underfive 数量追加
    ...
    protected void writeMainForRanking() throws Exception {
        ...
        if (!this.reportList.isEmpty()) {
            ...
            for (OriginalReport01DetailBean detail : summaryList) {
                ...
                // R3
                super.writeValue(3, rowIndex, nullToZero(detail.getR3().getRate()));

                // R全（新規・再来含）
                super.writeValue(4, rowIndex, nullToZero(detail.getRr().getRate()));

                // TO率（時間超過率）
                super.writeValue(5, rowIndex, nullToZero(detail.getToRate().getRate()));

                // 客数
                super.writeValue(6, rowIndex, nullToZero(detail.getSalesNum().getNum()));

                // 指名数
                super.writeValue(7, rowIndex, nullToZero(detail.getDesignatedNum().getNum()));

                // 口コミ数（良い）
                super.writeValue(8, rowIndex, nullToZero(detail.getGoodNum().getNum()));

                // 口コミ数（悪い）
                super.writeValue(9, rowIndex, nullToZero(detail.getBadNum().getNum()));

                // お直し数、お直し率
                super.setRate(detail.getRetouchNumEachSlip(), detail.getSalesNum().getNum());
                super.writeValue(10, rowIndex, nullToZero(detail.getRetouchNum().getNum()));
                super.writeValue(11, rowIndex, nullToZero(detail.getRetouchNumEachSlip().getRate()));

                // IVS_LVTu start edit 20210129 #18378 underfive 数量追加
                // underfive-数量
                super.writeValue(12, rowIndex, nullToZero(detail.getUnderfiveQuantity().getNum()));

                // underfive【プラチナ割引】数量
                super.writeValue(13, rowIndex, nullToZero(detail.getUnderfiveDiscountQuantity().getNum()));
                // 201710 GB edit end #27108
                // IVS_LVTu end edit 20210129 #18378 underfive 数量追加
                // 単価
                super.writeValue(14, rowIndex, nullToZero(detail.getProductNum().getNum()));

                // 売上
                super.writeValue(15, rowIndex, nullToZero(detail.getProceedsNum().getNum()));
...
//                // fromto選択時は最後の年月のデータがトータルの値になっているため。
            ...
            // 201710 GB add start #27108 R3追加
            detail.getTwiceSalesNum().setNum(d.get(lastYm).getTwiceSalesNum().getNum());
            detail.getR3().setNum(d.get(lastYm).getR3().getNum());
            // 201710 GB add end #27108 R3追加
//            }
            ...
            // 201710 GB add start #27108 R3追加
            super.setRate(detail.getR3(), detail.getTwiceSalesNum().getNum());
            // 201710 GB add end #27108 R3追加
            // R全（新規・再来含）
    ...
    private void sortByOrder(List<OriginalReport01DetailBean> summaryDetailList) {
        ...
        case R3:
            Collections.sort(summaryDetailList, new Comparator<OriginalReport01DetailBean>() {
                @Override
                public int compare(OriginalReport01DetailBean o1, OriginalReport01DetailBean o2) {
                    return nullToZero(o2.getR3().getRate()).compareTo(nullToZero(o1.getR3().getRate()));
                }
            });
            break;
        // 201710 GB add end #27108 R3追加
        ...
        case PROCEEDS:
            ...
            break;
        // IVS_LVTu start edit 20210129 #18378 underfive 数量追加
        case UNDERFIVE_QUANTITY:
            Collections.sort(summaryDetailList, new Comparator<OriginalReport01DetailBean>() {
                @Override
                public int compare(OriginalReport01DetailBean o1, OriginalReport01DetailBean o2) {
                    return nullToZero(o2.getUnderfiveQuantity().getNum()).compareTo(nullToZero(o1.getUnderfiveQuantity().getNum()));
                }
            });
            break;
        case UNDERFIVE_DISCOUNT_QUANTITY:
            Collections.sort(summaryDetailList, new Comparator<OriginalReport01DetailBean>() {
                @Override
                public int compare(OriginalReport01DetailBean o1, OriginalReport01DetailBean o2) {
                    return nullToZero(o2.getUnderfiveDiscountQuantity().getNum()).compareTo(nullToZero(o1.getUnderfiveDiscountQuantity().getNum()));
                }
            });
            break;
        // IVS_LVTu end edit 20210129 #18378 underfive 数量追加
    ...
    protected void writeMainForMonthTransition() throws Exception {
        ...
        // 対象期間対象の出力を行う
        String result[] = getSheetNames ( OUTPUT_TARGET_COLUMNS, OUTPUT_UNDERFIVE_COLUMNS) ;
        this.writeMainForMonthTransitionDetail(this.getTargetYmList(), result);
                ...
                if (!this.reportList.isEmpty()) {
                    ...
                    for (String column : columns) {
                        ...
                        for (OriginalListBean bean : beans) {
                            ...
                            } else if (column.equals(OUTPUT_PERIOD_COLUMNS[2])) {
                                // R（既存再来）の場合は既存顧客数で算出する。
                                List<OriginalListBean> allNum = columnsDetail.get(OUTPUT_OTHER_COLUMNS[2]);
    ...
    private void addColumnsDetail(Map<String, List<OriginalListBean>> columnsDetail, OriginalReport01DetailBean row) {
        ...
        this.addColumnsDetail(columnsDetail, row.getR3(), OUTPUT_PERIOD_COLUMNS[1]); // 201710 GB #27108 R3追加
        this.addColumnsDetail(columnsDetail, row.getRr(), OUTPUT_PERIOD_COLUMNS[2]);
        ...
        // IVS_LVTu start edit 20210129 #18378 underfive 数量追加
        this.addColumnsDetail(columnsDetail, row.getUnderfiveQuantity(), OUTPUT_TARGET_COLUMNS[7]);
        this.addColumnsDetail(columnsDetail, row.getUnderfiveDiscountQuantity(), OUTPUT_TARGET_COLUMNS[8]);
        // IVS_LVTu end edit 20210129 #18378 underfive 数量追加
        this.addColumnsDetail(columnsDetail, row.getProductNum(), OUTPUT_TARGET_COLUMNS[9]);
        this.addColumnsDetail(columnsDetail, row.getProceedsNum(), OUTPUT_TARGET_COLUMNS[10]);

        this.addColumnsDetail(columnsDetail, row.getNewSalesNum(), OUTPUT_OTHER_COLUMNS[0]);
        this.addColumnsDetail(columnsDetail, row.getTwiceSalesNum(), OUTPUT_OTHER_COLUMNS[1]); // 201710 GB #27108 R3追加
        this.addColumnsDetail(columnsDetail, row.getFixedSalesNum(), OUTPUT_OTHER_COLUMNS[2]);
    }
...
//                    new OriginalListBean(null, rs.getLong("designated_num")));
    ...
    private void setSalesNumRR(
        ...
        while (targetYmIterator.hasNext()) {
            ...
            while (rs.next()) {
                ...
                // 201710 GB add start #27108
                // 二回目顧客数
                row.getDetail().get(targetYm).setTwiceSalesNum(
                        new OriginalListBean(null, rs.getLong("all_twice_num")));
                // R3
                row.getDetail().get(targetYm).setR3(
                    new OriginalListBean(null, rs.getLong("r3_num")));
                // 201710 GB add end #27108
            }
            rs.close();
        }
    }
	...
	sb.append("        ) ");
        // 20170707 GB Start Add #17754 [GB内対応][gb]シンクアクトオリジナル帳票のお直し数のカウント集計ロジックの変更
        sb.append("     INNER JOIN mst_staff AS t3 ON t2.staff_id = t3.staff_id ");
        // 20170707 GB End Add #17754 [GB内対応][gb]シンクアクトオリジナル帳票のお直し数のカウント集計ロジックの変更
	sb.append("    WHERE ");
	...
	if (row.getShopId() != null) {
            ...
            //20180111 GB edit start #17754 feedback
            //sb.append("   AND t3.shop_id = ").append(SQLUtil.convertForSQL(row.getShopId()));
            if (super.getBean().isShopFlag()) {
                    sb.append("   AND t1.shop_id = ").append(SQLUtil.convertForSQL(row.getShopId()));
            } else {
                    sb.append("   AND t3.shop_id = ").append(SQLUtil.convertForSQL(row.getShopId()));
            }
            //20180111 GB edit end #17754 feedback
     ...
     * @throws Exception 汎用例外
     */
    private void setUnderfiveNum(Map<Integer, Map<String, DataUnderfive>> underfiveNumMap, OriginalReport01Bean row) throws Exception {
        Integer id;
        if (super.getBean().isShopFlag()) {
            id = row.getShopId();
        } else {
        ...
        Map<String, DataUnderfive> underfiveNumMapYm = underfiveNumMap.get(id);
        if(underfiveNumMapYm == null) {
            return;
        }
        for(String salesYm : underfiveNumMapYm.keySet()) {
            ...
            OriginalReport01DetailBean bean = row.getDetail().get(salesYm);
            DataUnderfive data = underfiveNumMapYm.get(salesYm);
            // underfive数量
            bean.setUnderfiveQuantity(new OriginalListBean(null, data.getUnderfiveQuantity()));
            // underfive【プラチナ割引】数量
            bean.setUnderfiveDiscountQuantity(new OriginalListBean(null, data.getUnderfiveDiscountQuantity()));
        }
    }
    // IVS_LVTu end edit 20210129 #18378 underfive 数量追加
    ...
    private String getRrR2Sql(OriginalReport01Bean row, Date targetYmdFrom, Date targetYmdTo, Date periodDateFrom, Date periodDateTo) {
        ...
        // 201710 GB add start #27108 R3追加
        sql.append("   COUNT(sub.all_twice_reappearance) AS all_twice_num, ");
        // 201710 GB add end #27108 R3追加
        sql.append("   COUNT(sub.r2_new_reappearance) AS r2_num,");
        sql.append("   COUNT(sub.rr_fixed_reappearance) AS rr_num");
        // 201710 GB add start #27108 R3追加
        sql.append("  , COUNT(sub.r3_new_reappearance) AS r3_num ");
        // 201710 GB add end #27108 R3追加
        sql.append(" FROM");
        sql.append(" (");
        sql.append(" select");
        ...
        sql.append("     else null");
        sql.append("     end");
        sql.append("     as all_twice_reappearance,");
        // 201710 GB add end #27108 R3追加
        sql.append("     case ");
        ...
        sql.append("     else null");
        sql.append("     end");
        sql.append("     as r3_new_reappearance ");
        // 201710 GB add end #27108 R3追加
     ...
     * underfive数量
     * @param conn
     * @param targetShopList
     * @param targetStaffList
     * @return
     * @throws Exception 
     */
    private Map<Integer, Map<String, DataUnderfive>> getUnderfiveNum(
            ConnectionWrapper conn, List<Integer> targetShopList, List<Integer> targetStaffList) throws Exception {
        StringBuilder query = new StringBuilder();
        query.append(" SELECT");
        query.append("   sub.sales_ym");
        query.append("   , sub.staff_id");
        query.append("   , sub.shop_id");
        query.append("   , SUM(CASE WHEN (sub.product_id = 2 AND sub.product_division = 4) THEN (- sub.product_num) WHEN (sub.product_id = 2 AND sub.product_division = 2) THEN sub.product_num END) AS underfiveQuantity ");
        query.append("   , SUM(CASE WHEN (sub.product_id = 4 AND sub.product_division = 4) THEN (- sub.product_num) WHEN (sub.product_id = 4 AND sub.product_division = 2) THEN sub.product_num END) AS underfiveDiscountQuantity ");
        query.append(" FROM (");
        query.append("   SELECT");
        query.append("     TO_CHAR(t1.sales_date, 'YYYY/MM') AS sales_ym");
        query.append("     , t1.shop_id");
        query.append("     , t2.product_division");
        query.append("     , t2.product_num AS product_num");
        query.append("     , t2.detail_staff_id AS staff_id");
        query.append("     , t2.product_id");
        query.append("   FROM");
        query.append("     data_sales AS t1");
        query.append("   INNER JOIN");
        query.append("     view_data_sales_detail_valid AS t2");
        query.append("     ON (t1.shop_id = t2.shop_id AND t1.slip_no = t2.slip_no)");
        query.append("   WHERE");
        query.append("     t2.product_division IN (2, 4)");
        ...
        Map<Integer, Map<String, DataUnderfive>> results = new LinkedHashMap<>();
        while (rs.next()) {
            Integer id;
            if(super.getBean().isShopFlag()) {
                id = rs.getInt("shop_id");
            ...
            if (!results.containsKey(id)) {
                results.put(id, new LinkedHashMap<String, DataUnderfive>());
            }

            String salesYm = rs.getString("sales_ym");
            Long underfiveNum = rs.getLong("underfiveQuantity");
            Long underfiveDiscountQuantity = rs.getLong("underfiveDiscountQuantity");
            DataUnderfive data = new DataUnderfive(underfiveNum, underfiveDiscountQuantity) ;
            if (results.get(id).containsKey(salesYm)) {
                DataUnderfive dataUnderfive = results.get(id).get(salesYm);
                
                underfiveNum = data.getUnderfiveQuantity() + dataUnderfive.getUnderfiveQuantity();
                underfiveDiscountQuantity = data.getUnderfiveDiscountQuantity() + dataUnderfive.getUnderfiveDiscountQuantity();
                
                data.setUnderfiveQuantity(underfiveNum);
                data.setUnderfiveDiscountQuantity(underfiveDiscountQuantity);
            } else {
                data.setUnderfiveQuantity(underfiveNum);
                data.setUnderfiveDiscountQuantity(underfiveDiscountQuantity);
            }
            results.get(id).put(salesYm, data);
        }
        return results;
    }
    
    /**
     * remove columns underfive 数量 and underfive【プラチナ割引】数量
     * @param parents
     * @param child
     * @return 
     */
    private String [] getSheetNames ( String [] parents, String [] child) {
        String [] results = null ;
        
        List<String> list = new ArrayList<>();
        Collections.addAll(list, parents);
        list.removeAll(Arrays.asList(child));
     ...
     * underfive数量
     */
    private class DataUnderfive {
        
        private Long underfiveQuantity ;
        private Long underfiveDiscountQuantity ;

        public DataUnderfive(Long underfiveQuantity, Long underfiveDiscountQuantity) {
            this.underfiveQuantity = underfiveQuantity;
            this.underfiveDiscountQuantity = underfiveDiscountQuantity;
        }
        
        public Long getUnderfiveQuantity() {
            return underfiveQuantity;
        }

        public void setUnderfiveQuantity(Long underfiveQuantity) {
            this.underfiveQuantity = underfiveQuantity;
        }

        public Long getUnderfiveDiscountQuantity() {
            return underfiveDiscountQuantity;
        }

        public void setUnderfiveDiscountQuantity(Long underfiveDiscountQuantity) {
            this.underfiveDiscountQuantity = underfiveDiscountQuantity;
        }
    }
    // IVS_LVTu end edit 20210129 #18378 underfive 数量追加
