...
import com.geobeck.sosia.pos.account.PrintReceipt.SubTaxesData;
...
public class HairInputAccountDetailDialog extends javax.swing.JDialog {
    ...
    private static final int  DIVITION_ZERO        = 0;
    private static final int  DIVITION_ONE         = 1;
    private static final int  DIVITION_TWO         = 2;
    private static final int  DIVITION_THREE       = 3;
    private static final int  DIVITION_FOUR        = 4;
    private static final int  DIVITION_FIVE        = 5;
    private static final int  DIVITION_SIX         = 6;
    private static final int  DIVITION_SEVEN       = 7;
    private static final int  DIVITION_EIGHT       = 8;
    private static final int  DIVITION_NINE        = 9;
    ...
    private void registButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_registButton2ActionPerformed
        ...
        if (checkInput()) {
            try {
				...
				//GB-D start modify 20181109 URL変更 -> 20190530 URL変更
				...
				//GB-D end modify 20181109 URL変更
                                    SystemInfo.getLogger().log(Level.SEVERE, e.getLocalizedMessage(), e);
                                    return;
                                }
                            } else {
    ...
    private void registButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_registButton3ActionPerformed
        ...
        for (DataSalesDetail dsd2 : ia.getSales()) {
            if (dsd2.getProductDivision().intValue() == 2) {
                ...
                jx.setCellValue(5, rowDiv2, ia.getAccountSetting().getDisplayValue(dsd2.getProductValue(), dsd2.getTaxRate()));
                jx.setCellValue(8, rowDiv2, dsd2.getProductNum());
                jx.setCellValue(9, rowDiv2, ia.getAccountSetting().getDisplayValue(dsd2.getProductValue(), dsd2.getTaxRate()) * dsd2.getProductNum());
                rowDiv2 = rowDiv2 + 1;
...
//                
                ...
                // IVS_LVTU edit 2019/08/28 #97064 [gb]SPOS増税対応
                jx.setCellValue(9, rowDiv5, ia.getAccountSetting().getDisplayValue(dsd2.getProductValue(), dsd2.getTaxRate()));
                rowDiv5 = rowDiv5 + 1;
            }
        }
        int iaddRow = 0;
    ...
    private void printReceipt() {
        ...
        // #43651 201803 GB start add 売上日付を表示するようにする
        pr.setSalesDate(ia.getSales().getSalesDate());
        prd.setSalesDate(ia.getSales().getSalesDate());
        // #43651 201803 GB end add 売上日付を表示するようにする
                ...
                if (dsd != null) {
                    if (dsd.getProductDivision() == 5) {
                        ...
                        pr.add(new ReceiptData(
                                ...
                                ia.getAccountSetting().getTax(dsd.getProductValue(), dsd.getDiscountValue(), dsd.getTaxRate()),
        ...
        for (DataSalesDetail dsd : ia.getSales()) {
            ...
            if (dsd.getProductDivision() == 5) {
                ...
                pr.add(new ReceiptData(
                        ...
                        dsd.getCourse().getCourseClass().getCourseClassName(),
                        // IVS_LVTU edit 2019/08/28 #97064 [gb]SPOS増税対応
                        this.getProductName(dsd, dsd.getCourse().getCourseName()),
                        dsd.getProductNum(),
                        getLoadBasePrice(dsd.getProductValue(), dsd.getTaxRate()),
                        //SystemInfo.getTax(dsd.getProductValue(), ia.getSales().getSalesDate()),
                        // IVS_LVTU edit 2019/08/28 #97064 [gb]SPOS増税対応
                        ia.getAccountSetting().getTax(dsd.getProductValue(), dsd.getDiscountValue(), dsd.getTaxRate()),
                        ia.getAccountSetting().getDisplayPriceType() == 1,
...
//                        ia.getAccountSetting().getDiscountType() == 1));
                ...
                pr.add(new ReceiptData(
                        ...
                        dsd.getProduct().getProductClass().getProductClassName(),
                        // IVS_LVTU edit 2019/08/28 #97064 [gb]SPOS増税対応
                        this.getProductName(dsd, dsd.getProduct().getProductName()),
                        dsd.getProductNum(),
                        getLoadBasePrice(dsd.getProductValue(), dsd.getTaxRate()),
                        //SystemInfo.getTax(dsd.getProductValue(), ia.getSales().getSalesDate()),
                        // IVS_LVTU edit 2019/08/28 #97064 [gb]SPOS増税対応
        ...
        pr.setSlipNo(ia.getSales().getSlipNo());
        // IVS_LVTU edit 2019/08/28 #97064 [gb]SPOS増税対応
        pr.setSubTaxList(this.getListTotalIncludeTax());
    ...
    private void printReceiptDigestion(PrintReceiptDigestion prd) {
        ...
        for (DataSalesDetail dsd : ia.getSales()) {
            ...
            if (dsd.getConsumptionCourse() != null) {
                // 20170807 nami edit start #21102
                ...
                System.out.println("伝票番号[" + dsd.getSlipNo() + "]");
                //setDigestionReceipt(dsd, prd, dcd.getSlipNo());
                setDigestionReceipt(dsd, prd);
                //  }
                
                
                break;  // 一回で伝票内の消化データ全て取得する
                // 20170807 nami edit end #21102
            }
        }
    ...
    //IVS_LVTu start add 2017/11/01 SPOSのカスタマイズ案件_SOSIA領収証
    /**
     * pos_hair_yoshieでかつレシート設定で「専用伝票」になっている場合の専用伝票を出力する
     * @param pr
     */
    private void printReceiptYoshie(PrintReceipt pr) {

        final int nineRowPage      = 11;     //  11 row on first page
        final int fifteenRowPage   = 18;     //  18 row on page 2
        List<DataSalesDetail> list = new ArrayList<DataSalesDetail>();
        boolean isFirstPage        = Boolean.FALSE; // Flag page first
        int indx = 0;
        int step = 0;
        int breakPage = nineRowPage;
        ...
        //割引
        DataSalesDetail discount = ia.getSales().getDiscount();
        ...
        // get Total page
        int totalPage = 1;
        if (list.size() > nineRowPage) {
            totalPage = totalPage + (list.size()-nineRowPage) / fifteenRowPage;
            totalPage = totalPage + (((list.size()-nineRowPage) % fifteenRowPage) > 0 ? 1 : 0);
        }

        for (int p = 0; p < totalPage; p++) {
            pr.setCustomer(ia.getSales().getCustomer());
            isFirstPage = Boolean.FALSE;
            if (p == 0) isFirstPage = Boolean.TRUE;

            pr.clear();
            while (indx < list.size()) {
                DataSalesDetail dsd = list.get(indx);
                if (dsd != null) {
                    long discountValue = 0l;
                    if (ia.getAccountSetting().getDiscountType() == 1) {
                        discountValue = (long)(dsd.getDiscountValue() * (1 + dsd.getTaxRate()));
                    } else {
                        discountValue = dsd.getDiscountValue();
                    }
                    if (dsd.getProductDivision() == DIVITION_ZERO) {
                        //割引
                        pr.add(new ReceiptData(
                                dsd.getProductDivision(),
                                dsd.getProduct().getProductClass().getProductClassName(),
                                "値引）" + dsd.getProduct().getProductName(),
                                dsd.getProductNum(),
                                0l,
                                ia.getAccountSetting().getTax(dsd.getProductValue(), dsd.getDiscountValue(), dsd.getTaxRate()),
                                ...
                                ia.getAccountSetting().getDisplayPriceType() == 1,
                                ia.getAccountSetting().getDiscountType() == 1));
                    } else if (dsd.getProductDivision() == DIVITION_FIVE) {
                        //コース契約の場合
                        pr.add(new ReceiptData(
                                dsd.getProductDivision(),
                                dsd.getCourse().getCourseClass().getCourseClassName(),
                                "コース）" + dsd.getCourse().getCourseName(),
                                dsd.getCourse().getNum(),
                                dsd.getProductValue(),
                                ia.getAccountSetting().getTax(dsd.getProductValue(), dsd.getDiscountValue(), dsd.getTaxRate()),
                                discountValue,
                                ia.getAccountSetting().getDisplayPriceType() == 1,
                                ia.getAccountSetting().getDiscountType() == 1));
                    } else if (dsd.getProductDivision() == DIVITION_SIX) {
                        //コース消化の場合
                        pr.add(new ReceiptData(
                                dsd.getProductDivision(),
                                dsd.getConsumptionCourse().getConsumptionCourseClass().getCourseClassName(),
                                "消化）" + dsd.getConsumptionCourse().getCourseName(),
                                dsd.getConsumptionNum(),
                                dsd.getConsumptionCourse().getConsumptionRestNum(),
                                0l,
                                SystemInfo.getTax(dsd.getProductValue(), ia.getSales().getSalesDate()),
                                ia.getAccountSetting().getDisplayPriceType() == 1,
                                ia.getAccountSetting().getDiscountType() == 1));
                        ...
                        String productName = "";
                        switch (dsd.getProductDivision()) {
                            case DIVITION_ONE:
                                productName = "治療）" + dsd.getProduct().getProductName();
                                break;
                            case DIVITION_TWO:
                                productName = "処方）" + dsd.getProduct().getProductName();
                                break;
                            case DIVITION_THREE:
                                productName = "返金）" + dsd.getProduct().getProductName();
                                break;
                            case DIVITION_FOUR:
                                productName = "返金）" + dsd.getProduct().getProductName();
                                break;
                            case DIVITION_SEVEN:
                                productName = "変更）" + dsd.getProduct().getProductName();
                                break;
                            case DIVITION_EIGHT:
                                productName = "解約）" + dsd.getProduct().getProductName();
                                break;
                            case DIVITION_NINE:
                                productName = "手数料）" + dsd.getProduct().getProductName();
                                break;
                            default:
                                productName = dsd.getProduct().getProductName();
                        }
                        pr.add(new ReceiptData(
                                dsd.getProductDivision(),
                                dsd.getProduct().getProductClass().getProductClassName(),
            ...
            pr.setPageNo((p + 1) + "/" + totalPage);
            pr.setLastPage((p + 1) == totalPage);
            pr.setFirstPage(isFirstPage);

            pr.print(MediaSizeName.ISO_A5, OrientationRequested.LANDSCAPE);
        }
    }
    //IVS_LVTu end add 2017/11/01 SPOSのカスタマイズ案件_SOSIA領収証
    ...
    private void reservationMenuUpdate(ConnectionWrapper con) {
        ...
        try {
            ...
            //IVS_LVTu start edit 2018/04/16 GB_Studio
            //fixed bug logic when product_division = 5 then remove on data_reservation
            // 予約データ登録
            sql.setLength(0);
            sql.append(" select");
            sql.append("      shop_id");
            ...
            sql.append("                 (");
            sql.append("                     select");
            sql.append("                         coalesce(sum(case a.product_division when 1 then b.operation_time when 5 then c.operation_time when 6 then c.operation_time end), 0)");
            sql.append("                     from");
            ...
            sql.append("                     where");
            sql.append("                             a.product_division in (1,5,6) ");
           ...
           sql.append("      ,(  ");
           sql.append("      case ");
           sql.append("              when (dsd.product_division = 5 or dsd.product_division = 6) then mc.operation_time ");
           ...
           sql.append("      ,(  ");
           sql.append("      case ");
           sql.append("              when dsd.product_division = 5 then 1 ");
           ...
           sql.append("      contract_shop_id   ");
            ...
            //IVS_LVTu end edit 2018/04/16 GB_Studio
    ...
    public PrintReceiptDigestion setDigestionReceipt(DataSalesDetail dsd, PrintReceiptDigestion pr) {
        // if (dsd.getConsumptionCourse().getContractNo() != null) {
        try {
            ConnectionWrapper con = SystemInfo.getConnection();
            ...
            // 20170807 nami edit start #21102 [GB内対応][gb] 役務消化確認書 契約していない契約が印字される
            ...
            ResultSetWrapper rs = con.executeQuery(pr.getReceiptDigestionSQL(dsd.getSlipNo(), dsd.getShop().getShopID()));            

            // 20170807 nami edit end #21102 [GB内対応][gb] 役務消化確認書 契約していない契約が印字される            
     ...
     * get product name
     * @param dsd
     * @param productNameBase
     * @return 
     */
    private String getProductName (DataSalesDetail dsd, String productNameBase) {
        if (productNameBase == null)
            return "";
        
        StringBuffer result = new StringBuffer();
        if (dsd != null && (dsd.getTaxRate() == null || dsd.getTaxRate().equals(ia.getTaxRate()))) {
            result.append(" ");
            result.append(productNameBase);
        } else {
            result.append("＊");
            result.append(productNameBase);
        }
    ...
    // IVS_LVTU start add 2019/08/28 #97064 [gb]SPOS増税対応
    /**
     * get List total and tax
     * @return 
     */
    private List getListTotalIncludeTax() {
        List<PrintReceipt.SubTaxesData> resultList = new ArrayList<PrintReceipt.SubTaxesData>();
        
        MstTaxs taxs = this.ia.getTaxs( this.ia.getSales().getSalesDate());
        Map<Double, Double> map = new TreeMap<Double, Double>(convertArray2Map(taxs));
         
        for(Map.Entry<Double, Double> entry : map.entrySet()) {
            Double key = entry.getKey();
            int taxRate = (int) (key * 100);

            String lableTax = String.valueOf(taxRate);
            Long sumTax = 0l;
            Long sumTotal = 0l;
            PrintReceipt.SubTaxesData data = new PrintReceipt().new SubTaxesData();
            boolean flagEntry = false;
            for (DataSalesDetail dsd : ia.getSales()) {

                if (dsd.getProductDivision() == 0 || dsd.getProductDivision() == 6) {
                    continue;
                }else if (dsd.getTaxRate() != null && dsd.getTaxRate().equals(key)) {
                    sumTotal = sumTotal + ia.getAccountSetting().getDisplayValue(dsd.getValue(), 
                            dsd.getProductNum().longValue(), dsd.getDiscountValue(), dsd.getTaxRate());
                    sumTax = sumTax + ia.getAccountSetting().getTax(dsd.getValue(), dsd.getDiscountValue(), dsd.getTaxRate());
                    flagEntry = true;
                }
            }
            
            if (flagEntry) {
                data.setTaxRateLabel(lableTax);
                data.setTargetTotal(sumTotal);
                data.setTax(sumTax);
                resultList.add(data);
            }
        }
     ...
     * @return 
     */
    private HashMap<Double, Double> convertArray2Map(MstTaxs taxs) {
        if (taxs == null)
            return new HashMap<Double, Double>();
        
        HashMap<Double, Double> map = new HashMap<Double, Double>();
        for (MstTax tax : taxs) {
            map.put(tax.getTaxRate(), tax.getTaxRate());
        }
        
        return map;
    }
    // IVS_LVTU end add 2019/08/28 #97064 [gb]SPOS増税対応
    
    /**
     * show Value 単価/金額
     * @param price
     * @param taxRateDetail
     * @param taxRateBase 軽減税率
     * @return 
     */
    private Long getLoadBasePrice(Long price, double taxRateDetail) {
        
        Long priceNew       = 0l;
        Long priceOldNoTax  = 0l;
        double taxRateBase = ia.getTaxRate();

        if (ia.getAccountSetting().getDisplayPriceType() == 1) {
            priceOldNoTax = ia.getAccountSetting().getPriceNoTax(price, taxRateDetail);
            priceNew = priceOldNoTax + Double.valueOf(priceOldNoTax * taxRateBase).longValue();
        }else {
            priceNew = price;
        }
